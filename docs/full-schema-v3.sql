-- =====================================================
-- SoundPub Dashboard - Full Database Schema Export v3.0
-- Updated: July 2026
-- Target schema: soundpub_dash
-- Target Supabase URL: https://supabase.carubra.com
-- Untuk migrasi ke Supabase Self-Hosted / local Supabase dengan schema non-public
-- =====================================================

-- =====================================================
-- BAGIAN 0: SCHEMA BOOTSTRAP
-- =====================================================
-- Catatan penting:
-- 1. Supabase Auth tetap memakai schema auth.
-- 2. Supabase Storage tetap memakai schema storage.
-- 3. App tables/functions/types dipindahkan ke schema soundpub_dash.
-- 4. Pastikan schema soundpub_dash diexpose di Supabase API/PostgREST.

CREATE SCHEMA IF NOT EXISTS soundpub_dash;

GRANT USAGE ON SCHEMA soundpub_dash TO anon, authenticated, service_role;
GRANT ALL ON SCHEMA soundpub_dash TO postgres, service_role;

SET search_path = soundpub_dash, public, auth, storage;
-- =====================================================
...
DO $$ BEGIN
  CREATE TYPE soundpub_dash.app_role AS ENUM (
    'superadmin', 
    'admin', 
    'label', 
    'whitelabel',
...
EXCEPTION
  WHEN duplicate_object THEN 
    ...
    ALTER TYPE soundpub_dash.app_role ADD VALUE IF NOT EXISTS 'whitelabel';
    ALTER TYPE soundpub_dash.app_role ADD VALUE IF NOT EXISTS 'copyright';
END $$;
...
CREATE TABLE IF NOT EXISTS soundpub_dash.profiles (
  ...
  parent_label_id UUID REFERENCES soundpub_dash.profiles(id),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

-- User roles table
CREATE TABLE IF NOT EXISTS soundpub_dash.user_roles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  role soundpub_dash.app_role NOT NULL DEFAULT 'user',
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  UNIQUE(user_id)
);

-- Artists table
CREATE TABLE IF NOT EXISTS soundpub_dash.artists (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  label_id UUID NOT NULL REFERENCES soundpub_dash.profiles(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
-- Releases table
CREATE TABLE IF NOT EXISTS soundpub_dash.releases (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  label_id UUID NOT NULL REFERENCES soundpub_dash.profiles(id),
  created_by UUID REFERENCES soundpub_dash.profiles(id),
  artist_user_id UUID REFERENCES soundpub_dash.profiles(id),
  upc TEXT,
  title TEXT NOT NULL,
...
-- Tracks table
CREATE TABLE IF NOT EXISTS soundpub_dash.tracks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  release_id UUID NOT NULL REFERENCES soundpub_dash.releases(id) ON DELETE CASCADE,
  artist_user_id UUID REFERENCES soundpub_dash.profiles(id),
  title TEXT NOT NULL,
  artist_name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS soundpub_dash.royalty_uploads (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES soundpub_dash.profiles(id),
...
-- Royalties table
CREATE TABLE IF NOT EXISTS soundpub_dash.royalties (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  upload_id UUID NOT NULL REFERENCES soundpub_dash.royalty_uploads(id) ON DELETE CASCADE,
  artist_user_id UUID,
...
CREATE TABLE IF NOT EXISTS soundpub_dash.composer_royalties (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS soundpub_dash.payout_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES soundpub_dash.profiles(id),
  ...
  status TEXT NOT NULL DEFAULT 'pending',
  notes TEXT,
  processed_by UUID REFERENCES soundpub_dash.profiles(id),
...
-- Audit logs table
CREATE TABLE IF NOT EXISTS soundpub_dash.audit_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- App settings table
CREATE TABLE IF NOT EXISTS soundpub_dash.app_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_profiles_parent_label ON soundpub_dash.profiles(parent_label_id);
CREATE INDEX IF NOT EXISTS idx_profiles_status ON soundpub_dash.profiles(status);
CREATE INDEX IF NOT EXISTS idx_profiles_email ON soundpub_dash.profiles(email);
CREATE INDEX IF NOT EXISTS idx_profiles_composer_code ON soundpub_dash.profiles(composer_code);
CREATE INDEX IF NOT EXISTS idx_user_roles_user_id ON soundpub_dash.user_roles(user_id);
CREATE INDEX IF NOT EXISTS idx_user_roles_role ON soundpub_dash.user_roles(role);
CREATE INDEX IF NOT EXISTS idx_artists_label_id ON soundpub_dash.artists(label_id);
CREATE INDEX IF NOT EXISTS idx_artists_name ON soundpub_dash.artists(name);
CREATE INDEX IF NOT EXISTS idx_releases_label_id ON soundpub_dash.releases(label_id);
CREATE INDEX IF NOT EXISTS idx_releases_status ON soundpub_dash.releases(status);
CREATE INDEX IF NOT EXISTS idx_releases_artist_name ON soundpub_dash.releases(artist_name);
CREATE INDEX IF NOT EXISTS idx_releases_artist_user_id ON soundpub_dash.releases(artist_user_id);
CREATE INDEX IF NOT EXISTS idx_releases_upc ON soundpub_dash.releases(upc);
CREATE INDEX IF NOT EXISTS idx_tracks_release_id ON soundpub_dash.tracks(release_id);
CREATE INDEX IF NOT EXISTS idx_tracks_isrc ON soundpub_dash.tracks(isrc);
CREATE INDEX IF NOT EXISTS idx_tracks_artist_user_id ON soundpub_dash.tracks(artist_user_id);
CREATE INDEX IF NOT EXISTS idx_royalty_uploads_user_id ON soundpub_dash.royalty_uploads(user_id);
CREATE INDEX IF NOT EXISTS idx_royalties_upload_id ON soundpub_dash.royalties(upload_id);
CREATE INDEX IF NOT EXISTS idx_royalties_period ON soundpub_dash.royalties(period);
CREATE INDEX IF NOT EXISTS idx_royalties_label_name ON soundpub_dash.royalties(label_name);
CREATE INDEX IF NOT EXISTS idx_royalties_isrc ON soundpub_dash.royalties(isrc);
CREATE INDEX IF NOT EXISTS idx_royalties_artist ON soundpub_dash.royalties(artist);
CREATE INDEX IF NOT EXISTS idx_royalties_artist_user_id ON soundpub_dash.royalties(artist_user_id);
CREATE INDEX IF NOT EXISTS idx_composer_royalties_composer_id ON soundpub_dash.composer_royalties(composer_id);
CREATE INDEX IF NOT EXISTS idx_composer_royalties_period ON soundpub_dash.composer_royalties(period);
CREATE INDEX IF NOT EXISTS idx_payout_requests_user_id ON soundpub_dash.payout_requests(user_id);
CREATE INDEX IF NOT EXISTS idx_payout_requests_status ON soundpub_dash.payout_requests(status);
CREATE INDEX IF NOT EXISTS idx_audit_logs_actor_id ON soundpub_dash.audit_logs(actor_id);
CREATE INDEX IF NOT EXISTS idx_audit_logs_created_at ON soundpub_dash.audit_logs(created_at);
CREATE INDEX IF NOT EXISTS idx_audit_logs_action ON soundpub_dash.audit_logs(action);
...
CREATE OR REPLACE FUNCTION soundpub_dash.has_role(_user_id UUID, _role app_role)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM soundpub_dash.user_roles
    WHERE user_id = _user_id AND role = _role
  )
$$;

-- Check if user is admin (superadmin or admin)
CREATE OR REPLACE FUNCTION soundpub_dash.is_admin(_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION soundpub_dash.is_whitelabel(_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- Get user role
CREATE OR REPLACE FUNCTION soundpub_dash.get_user_role(_user_id UUID)
RETURNS app_role
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT role FROM soundpub_dash.user_roles
  WHERE user_id = _user_id
  LIMIT 1
$$;

-- Get user full name
CREATE OR REPLACE FUNCTION soundpub_dash.get_user_full_name(_user_id UUID)
RETURNS TEXT
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT full_name FROM soundpub_dash.profiles
  WHERE id = _user_id
$$;

-- Get user parent label id
CREATE OR REPLACE FUNCTION soundpub_dash.get_user_parent_label_id(_user_id UUID)
RETURNS UUID
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT parent_label_id FROM soundpub_dash.profiles
  WHERE id = _user_id
$$;

-- Get release label ids for user
CREATE OR REPLACE FUNCTION soundpub_dash.get_user_release_label_ids(_user_id UUID)
RETURNS SETOF UUID
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT DISTINCT label_id FROM soundpub_dash.releases
  WHERE label_id = _user_id 
     OR artist_name = (SELECT full_name FROM soundpub_dash.profiles WHERE id = _user_id)
$$;

-- Get artist user_id by name (for matching)
CREATE OR REPLACE FUNCTION soundpub_dash.get_artist_user_id_by_name(_artist_name TEXT, _label_id UUID DEFAULT NULL)
RETURNS UUID
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_stats()
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_monthly_summary()
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_platform_summary(_limit INTEGER DEFAULT 10)
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_country_summary(_limit INTEGER DEFAULT 10)
...
-- Get royalty periods
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_periods()
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_period_summary()
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_comparison(_current_periods TEXT[], _previous_periods TEXT[])
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_top_performers(_current_periods TEXT[], _previous_periods TEXT[], _group_by TEXT DEFAULT 'title', _limit INTEGER DEFAULT 10)
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_label_breakdown(_period TEXT DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_artist_breakdown(_period TEXT DEFAULT NULL, _limit INTEGER DEFAULT 20)
...
CREATE OR REPLACE FUNCTION soundpub_dash.get_royalty_track_breakdown(_period TEXT DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION soundpub_dash.handle_new_user()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  INSERT INTO soundpub_dash.profiles (id, email, full_name, password_set)
  VALUES (
    NEW.id, 
    NEW.email, 
  ...
  INSERT INTO soundpub_dash.user_roles (user_id, role)
  VALUES (NEW.id, 'user');
...
CREATE OR REPLACE FUNCTION soundpub_dash.update_timestamp()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION soundpub_dash.update_balance_on_payout_status_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
    caller_is_admin := soundpub_dash.is_admin(auth.uid());
...
CREATE TRIGGER on_auth_user_created
  ...
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.handle_new_user();

-- Update timestamp triggers
DROP TRIGGER IF EXISTS update_profiles_timestamp ON soundpub_dash.profiles;
CREATE TRIGGER update_profiles_timestamp
  BEFORE UPDATE ON soundpub_dash.profiles
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_artists_timestamp ON soundpub_dash.artists;
CREATE TRIGGER update_artists_timestamp
  BEFORE UPDATE ON soundpub_dash.artists
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_releases_timestamp ON soundpub_dash.releases;
CREATE TRIGGER update_releases_timestamp
  BEFORE UPDATE ON soundpub_dash.releases
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_tracks_timestamp ON soundpub_dash.tracks;
CREATE TRIGGER update_tracks_timestamp
  BEFORE UPDATE ON soundpub_dash.tracks
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_royalty_uploads_timestamp ON soundpub_dash.royalty_uploads;
CREATE TRIGGER update_royalty_uploads_timestamp
  BEFORE UPDATE ON soundpub_dash.royalty_uploads
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_payout_requests_timestamp ON soundpub_dash.payout_requests;
CREATE TRIGGER update_payout_requests_timestamp
  BEFORE UPDATE ON soundpub_dash.payout_requests
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

DROP TRIGGER IF EXISTS update_composer_royalties_timestamp ON soundpub_dash.composer_royalties;
CREATE TRIGGER update_composer_royalties_timestamp
  BEFORE UPDATE ON soundpub_dash.composer_royalties
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_timestamp();

-- Payout balance update trigger
DROP TRIGGER IF EXISTS on_payout_status_change ON soundpub_dash.payout_requests;
CREATE TRIGGER on_payout_status_change
  BEFORE UPDATE ON soundpub_dash.payout_requests
  FOR EACH ROW EXECUTE FUNCTION soundpub_dash.update_balance_on_payout_status_change();

-- =====================================================
-- BAGIAN 7: ENABLE ROW LEVEL SECURITY
-- =====================================================

ALTER TABLE soundpub_dash.profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.user_roles ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.artists ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.releases ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.tracks ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.royalty_uploads ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.royalties ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.composer_royalties ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.payout_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.audit_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE soundpub_dash.app_settings ENABLE ROW LEVEL SECURITY;

-- =====================================================
-- BAGIAN 8: RLS POLICIES - PROFILES
-- =====================================================

DROP POLICY IF EXISTS "Users can view their own profile" ON soundpub_dash.profiles;
CREATE POLICY "Users can view their own profile" ON soundpub_dash.profiles
  FOR SELECT USING (id = auth.uid());

DROP POLICY IF EXISTS "Admins can view all profiles" ON soundpub_dash.profiles;
CREATE POLICY "Admins can view all profiles" ON soundpub_dash.profiles
  FOR SELECT USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Admins can manage all profiles" ON soundpub_dash.profiles;
CREATE POLICY "Admins can manage all profiles" ON soundpub_dash.profiles
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can update their own profile" ON soundpub_dash.profiles;
CREATE POLICY "Users can update their own profile" ON soundpub_dash.profiles
  FOR UPDATE USING (id = auth.uid());

DROP POLICY IF EXISTS "Labels can view their artists" ON soundpub_dash.profiles;
CREATE POLICY "Labels can view their artists" ON soundpub_dash.profiles
  FOR SELECT USING (has_role(auth.uid(), 'label') AND parent_label_id = auth.uid());

DROP POLICY IF EXISTS "Labels can update their artists" ON soundpub_dash.profiles;
CREATE POLICY "Labels can update their artists" ON soundpub_dash.profiles
  FOR UPDATE 
...
DROP POLICY IF EXISTS "Whitelabels can view their artists" ON soundpub_dash.profiles;
CREATE POLICY "Whitelabels can view their artists" ON soundpub_dash.profiles
  FOR SELECT USING (has_role(auth.uid(), 'whitelabel') AND parent_label_id = auth.uid());

DROP POLICY IF EXISTS "Whitelabels can update their artists" ON soundpub_dash.profiles;
CREATE POLICY "Whitelabels can update their artists" ON soundpub_dash.profiles
  FOR UPDATE 
...
DROP POLICY IF EXISTS "Artists can view their parent label profile" ON soundpub_dash.profiles;
CREATE POLICY "Artists can view their parent label profile" ON soundpub_dash.profiles
  FOR SELECT USING (has_role(auth.uid(), 'artist') AND id = get_user_parent_label_id(auth.uid()));

DROP POLICY IF EXISTS "Users can view label profiles for their releases" ON soundpub_dash.profiles;
CREATE POLICY "Users can view label profiles for their releases" ON soundpub_dash.profiles
...
DROP POLICY IF EXISTS "Users can view their own roles" ON soundpub_dash.user_roles;
CREATE POLICY "Users can view their own roles" ON soundpub_dash.user_roles
  FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Admins can manage all roles" ON soundpub_dash.user_roles;
CREATE POLICY "Admins can manage all roles" ON soundpub_dash.user_roles
  FOR ALL USING (is_admin(auth.uid()));
...
DROP POLICY IF EXISTS "Admins can manage all artists" ON soundpub_dash.artists;
CREATE POLICY "Admins can manage all artists" ON soundpub_dash.artists
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Labels can manage their own artists" ON soundpub_dash.artists;
CREATE POLICY "Labels can manage their own artists" ON soundpub_dash.artists
  FOR ALL 
  USING (label_id = auth.uid())
  WITH CHECK (label_id = auth.uid());

DROP POLICY IF EXISTS "Whitelabels can manage their own artists" ON soundpub_dash.artists;
CREATE POLICY "Whitelabels can manage their own artists" ON soundpub_dash.artists
  FOR ALL 
  USING (has_role(auth.uid(), 'whitelabel') AND label_id = auth.uid())
  WITH CHECK (has_role(auth.uid(), 'whitelabel') AND label_id = auth.uid());

DROP POLICY IF EXISTS "Admins, labels and associated artists can view artists" ON soundpub_dash.artists;
CREATE POLICY "Admins, labels and associated artists can view artists" ON soundpub_dash.artists
  FOR SELECT USING (
    is_admin(auth.uid())
    OR label_id = auth.uid()
    OR EXISTS (
      SELECT 1 FROM soundpub_dash.profiles p
      WHERE p.id = auth.uid()
...
DROP POLICY IF EXISTS "Admins can manage all releases" ON soundpub_dash.releases;
CREATE POLICY "Admins can manage all releases" ON soundpub_dash.releases
  FOR ALL 
  USING (is_admin(auth.uid()))
  WITH CHECK (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Labels can manage their releases" ON soundpub_dash.releases;
CREATE POLICY "Labels can manage their releases" ON soundpub_dash.releases
  FOR ALL 
  USING (has_role(auth.uid(), 'label') AND label_id = auth.uid())
  WITH CHECK (has_role(auth.uid(), 'label') AND label_id = auth.uid());

DROP POLICY IF EXISTS "Whitelabels can manage their releases" ON soundpub_dash.releases;
CREATE POLICY "Whitelabels can manage their releases" ON soundpub_dash.releases
  FOR ALL 
  USING (has_role(auth.uid(), 'whitelabel') AND label_id = auth.uid())
  WITH CHECK (has_role(auth.uid(), 'whitelabel') AND label_id = auth.uid());

DROP POLICY IF EXISTS "Artists can view their releases" ON soundpub_dash.releases;
CREATE POLICY "Artists can view their releases" ON soundpub_dash.releases
  FOR SELECT 
  USING (
...
DROP POLICY IF EXISTS "Artists can insert their releases" ON soundpub_dash.releases;
CREATE POLICY "Artists can insert their releases" ON soundpub_dash.releases
  FOR INSERT 
  WITH CHECK (
...
DROP POLICY IF EXISTS "Artists can update their releases" ON soundpub_dash.releases;
CREATE POLICY "Artists can update their releases" ON soundpub_dash.releases
  FOR UPDATE 
  USING (
    has_role(auth.uid(), 'artist') AND (
      artist_user_id = auth.uid()
...
DROP POLICY IF EXISTS "Admins can manage all tracks" ON soundpub_dash.tracks;
CREATE POLICY "Admins can manage all tracks" ON soundpub_dash.tracks
  FOR ALL 
  USING (is_admin(auth.uid()))
  WITH CHECK (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Labels can manage tracks for their releases" ON soundpub_dash.tracks;
CREATE POLICY "Labels can manage tracks for their releases" ON soundpub_dash.tracks
  FOR ALL 
...
DROP POLICY IF EXISTS "Whitelabels can manage tracks for their releases" ON soundpub_dash.tracks;
CREATE POLICY "Whitelabels can manage tracks for their releases" ON soundpub_dash.tracks
  FOR ALL 
...
DROP POLICY IF EXISTS "Artists can view their tracks" ON soundpub_dash.tracks;
CREATE POLICY "Artists can view their tracks" ON soundpub_dash.tracks
  FOR SELECT 
  USING (
    has_role(auth.uid(), 'artist') AND (
      artist_user_id = auth.uid()
...
DROP POLICY IF EXISTS "Artists can insert tracks for their releases" ON soundpub_dash.tracks;
CREATE POLICY "Artists can insert tracks for their releases" ON soundpub_dash.tracks
  FOR INSERT 
  WITH CHECK (
    has_role(auth.uid(), 'artist')
    AND EXISTS (
...
DROP POLICY IF EXISTS "Artists can update tracks for their releases" ON soundpub_dash.tracks;
CREATE POLICY "Artists can update tracks for their releases" ON soundpub_dash.tracks
  FOR UPDATE 
  USING (
    has_role(auth.uid(), 'artist')
    AND EXISTS (
...
DROP POLICY IF EXISTS "Admins can manage all uploads" ON soundpub_dash.royalty_uploads;
CREATE POLICY "Admins can manage all uploads" ON soundpub_dash.royalty_uploads
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can view their own uploads" ON soundpub_dash.royalty_uploads;
CREATE POLICY "Users can view their own uploads" ON soundpub_dash.royalty_uploads
  FOR SELECT USING (user_id = auth.uid());
...
DROP POLICY IF EXISTS "Admins can manage all royalties" ON soundpub_dash.royalties;
CREATE POLICY "Admins can manage all royalties" ON soundpub_dash.royalties
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Labels can view royalties for their artists" ON soundpub_dash.royalties;
CREATE POLICY "Labels can view royalties for their artists" ON soundpub_dash.royalties
  FOR SELECT 
  USING (has_role(auth.uid(), 'label') AND label_name = get_user_full_name(auth.uid()));

DROP POLICY IF EXISTS "Whitelabels can view royalties for their artists" ON soundpub_dash.royalties;
CREATE POLICY "Whitelabels can view royalties for their artists" ON soundpub_dash.royalties
  FOR SELECT 
  USING (has_role(auth.uid(), 'whitelabel') AND label_name = get_user_full_name(auth.uid()));

DROP POLICY IF EXISTS "Artists can view their royalties" ON soundpub_dash.royalties;
CREATE POLICY "Artists can view their royalties" ON soundpub_dash.royalties
  FOR SELECT 
  USING (
    has_role(auth.uid(), 'artist') AND (
      artist_user_id = auth.uid()
...
DROP POLICY IF EXISTS "Admins can manage all composer royalties" ON soundpub_dash.composer_royalties;
CREATE POLICY "Admins can manage all composer royalties" ON soundpub_dash.composer_royalties
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Copyright users can view composer royalties" ON soundpub_dash.composer_royalties;
CREATE POLICY "Copyright users can view composer royalties" ON soundpub_dash.composer_royalties
...
DROP POLICY IF EXISTS "Admins can manage all payouts" ON soundpub_dash.payout_requests;
CREATE POLICY "Admins can manage all payouts" ON soundpub_dash.payout_requests
  FOR ALL USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Users can view their own payouts" ON soundpub_dash.payout_requests;
CREATE POLICY "Users can view their own payouts" ON soundpub_dash.payout_requests
  FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can create pending payouts" ON soundpub_dash.payout_requests;
CREATE POLICY "Users can create pending payouts" ON soundpub_dash.payout_requests
  FOR INSERT WITH CHECK (user_id = auth.uid() AND status = 'pending' AND amount > 0);

DROP POLICY IF EXISTS "Users can cancel pending payouts" ON soundpub_dash.payout_requests;
CREATE POLICY "Users can cancel pending payouts" ON soundpub_dash.payout_requests
...
DROP POLICY IF EXISTS "Admins can view all audit logs" ON soundpub_dash.audit_logs;
CREATE POLICY "Admins can view all audit logs" ON soundpub_dash.audit_logs
  FOR SELECT USING (is_admin(auth.uid()));

DROP POLICY IF EXISTS "Service role can insert audit logs" ON soundpub_dash.audit_logs;
CREATE POLICY "Service role can insert audit logs" ON soundpub_dash.audit_logs
...
DROP POLICY IF EXISTS "Anyone can view app settings" ON soundpub_dash.app_settings;
CREATE POLICY "Anyone can view app settings" ON soundpub_dash.app_settings
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Superadmins can manage app settings" ON soundpub_dash.app_settings;
CREATE POLICY "Superadmins can manage app settings" ON soundpub_dash.app_settings
...
INSERT INTO soundpub_dash.app_settings (key, value) VALUES 
...
CREATE TABLE IF NOT EXISTS soundpub_dash.release_payments (
...
CREATE INDEX IF NOT EXISTS idx_release_payments_release_id ON soundpub_dash.release_payments(release_id);
CREATE INDEX IF NOT EXISTS idx_release_payments_user_id ON soundpub_dash.release_payments(user_id);
CREATE INDEX IF NOT EXISTS idx_release_payments_status ON soundpub_dash.release_payments(status);
CREATE INDEX IF NOT EXISTS idx_release_payments_xendit_invoice_id ON soundpub_dash.release_payments(xendit_invoice_id);

ALTER TABLE soundpub_dash.release_payments ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own payments"
ON soundpub_dash.release_payments FOR SELECT TO authenticated
USING (user_id = auth.uid());

CREATE POLICY "Users can insert their own payments"
ON soundpub_dash.release_payments FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

CREATE POLICY "Admins can manage all payments"
ON soundpub_dash.release_payments FOR ALL TO authenticated
USING (is_admin(auth.uid()));

CREATE TRIGGER update_release_payments_timestamp
  BEFORE UPDATE ON soundpub_dash.release_payments
  FOR EACH ROW EXECUTE FUNCTION update_timestamp();

-- Harga per track (configurable)
INSERT INTO soundpub_dash.app_settings (key, value) VALUES ('release_price_per_track', '50000')
ON CONFLICT (key) DO NOTHING;
...
-- =====================================================
-- APPENDIX v2.4 â€” Delta Juniâ€“Juli 2026
...
DROP POLICY IF EXISTS "Users can insert own notifications" ON soundpub_dash.notifications;
CREATE POLICY "Users can insert own notifications"
  ON soundpub_dash.notifications
  FOR INSERT
  TO authenticated
  WITH CHECK (
    soundpub_dash.is_admin(auth.uid())
    OR (user_id = auth.uid() AND COALESCE(is_global, false) = false)
  );

-- Trigger functions: revoke direct EXECUTE dari anon/PUBLIC
REVOKE EXECUTE ON FUNCTION soundpub_dash.handle_new_user() FROM anon, PUBLIC;
REVOKE EXECUTE ON FUNCTION soundpub_dash.prevent_profile_privilege_escalation() FROM anon, PUBLIC;
REVOKE EXECUTE ON FUNCTION soundpub_dash.update_balance_on_payout_status_change() FROM anon, PUBLIC;
REVOKE EXECUTE ON FUNCTION soundpub_dash.update_timestamp() FROM anon, PUBLIC;
...
CREATE POLICY "Users can upload release covers to own folder"
  ...
  WITH CHECK (
    ...
    AND (
      soundpub_dash.has_role(auth.uid(), 'label') OR soundpub_dash.has_role(auth.uid(), 'artist')
      OR soundpub_dash.has_role(auth.uid(), 'whitelabel') OR soundpub_dash.is_admin(auth.uid())
    )
  );
...
CREATE POLICY "Owners and admins can update release-covers"
  ...
  USING (
    bucket_id = 'release-covers'
    AND (soundpub_dash.is_admin(auth.uid()) OR (storage.foldername(name))[1] = auth.uid()::text)
  );
...
CREATE POLICY "Users can upload audio clips to own folder"
  ...
  WITH CHECK (
    ...
    AND (
      soundpub_dash.is_admin(auth.uid())
      OR soundpub_dash.has_role(auth.uid(), 'label')
      OR soundpub_dash.has_role(auth.uid(), 'whitelabel')
      OR soundpub_dash.has_role(auth.uid(), 'artist')
    )
  );
...
DROP POLICY IF EXISTS "Authenticated users can insert own audit logs" ON soundpub_dash.audit_logs;

-- ---- 4) label_profile_update_safe helper ----
CREATE OR REPLACE FUNCTION soundpub_dash.label_profile_update_safe(
  _id uuid,
  _balance numeric,
...
$$;
REVOKE EXECUTE ON FUNCTION soundpub_dash.label_profile_update_safe(uuid,numeric,numeric,numeric,text,uuid,text,text,text,text,text,text,boolean) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION soundpub_dash.label_profile_update_safe(uuid,numeric,numeric,numeric,text,uuid,text,text,text,text,text,text,boolean) TO authenticated;

-- Profiles: WITH CHECK guard di self-update + label/whitelabel update artist
DROP POLICY IF EXISTS "Users can update their own profile" ON soundpub_dash.profiles;
CREATE POLICY "Users can update their own profile"
  ON soundpub_dash.profiles FOR UPDATE TO authenticated
  USING (id = auth.uid())
  WITH CHECK (
    id = auth.uid()
    AND soundpub_dash.label_profile_update_safe(
...
CREATE POLICY "Labels can update their artists (safe fields)"
  ON soundpub_dash.profiles FOR UPDATE TO authenticated
  USING (soundpub_dash.has_role(auth.uid(), 'label') AND parent_label_id = auth.uid())
  WITH CHECK (
    soundpub_dash.has_role(auth.uid(), 'label') AND parent_label_id = auth.uid()
    AND soundpub_dash.label_profile_update_safe(
      id, balance, label_revenue, artist_revenue,
      subscription_status, parent_label_id, composer_code,
      status, email, sso_provider, sso_user_id, sso_user_type, password_set
...
CREATE POLICY "Whitelabels can update their artists (safe fields)"
  ON soundpub_dash.profiles FOR UPDATE TO authenticated
  USING (soundpub_dash.has_role(auth.uid(), 'whitelabel') AND parent_label_id = auth.uid())
  WITH CHECK (
    soundpub_dash.has_role(auth.uid(), 'whitelabel') AND parent_label_id = auth.uid()
    AND soundpub_dash.label_profile_update_safe(
      id, balance, label_revenue, artist_revenue,
      subscription_status, parent_label_id, composer_code,
      status, email, sso_provider, sso_user_id, sso_user_type, password_set
...
CREATE POLICY "Only admins can update payouts"
  ON soundpub_dash.payout_requests AS RESTRICTIVE FOR UPDATE TO authenticated
  USING (soundpub_dash.is_admin(auth.uid()))
  WITH CHECK (soundpub_dash.is_admin(auth.uid()));

-- ---- 5) Email notification opt-in + email_send_log (2026-06-24) ----
ALTER TABLE soundpub_dash.profiles
...
CREATE TABLE IF NOT EXISTS soundpub_dash.email_send_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
GRANT SELECT ON soundpub_dash.email_send_log TO authenticated;
GRANT ALL ON soundpub_dash.email_send_log TO service_role;
ALTER TABLE soundpub_dash.email_send_log ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Admins can view email send log" ON soundpub_dash.email_send_log;
CREATE POLICY "Admins can view email send log"
  ON soundpub_dash.email_send_log FOR SELECT TO authenticated
  USING (soundpub_dash.is_admin(auth.uid()));
CREATE INDEX IF NOT EXISTS idx_email_send_log_created_at
  ON soundpub_dash.email_send_log (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_email_send_log_template
  ON soundpub_dash.email_send_log (template_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_email_send_log_idem
  ON soundpub_dash.email_send_log (idempotency_key)
  WHERE idempotency_key IS NOT NULL;

-- ---- 6) Royalties: label_user_id stable identifier (2026-06-25) ----
ALTER TABLE soundpub_dash.royalties
  ADD COLUMN IF NOT EXISTS label_user_id uuid REFERENCES soundpub_dash.profiles(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_royalties_label_user_id ON soundpub_dash.royalties(label_user_id);

-- Backfill dari full_name unik
WITH unique_labels AS (
  SELECT lower(trim(p.full_name)) AS name_key, MIN(p.id::text)::uuid AS only_id
  FROM soundpub_dash.profiles p
  JOIN soundpub_dash.user_roles ur ON ur.user_id = p.id
  WHERE ur.role IN ('label','whitelabel')
    AND p.full_name IS NOT NULL AND trim(p.full_name) <> ''
  GROUP BY 1
  HAVING COUNT(*) = 1
)
UPDATE soundpub_dash.royalties r
...
CREATE POLICY "Labels can view royalties for their artists"
  ON soundpub_dash.royalties FOR SELECT TO authenticated
  USING (soundpub_dash.has_role(auth.uid(), 'label') AND label_user_id = auth.uid());

DROP POLICY IF EXISTS "Whitelabels can view royalties for their artists" ON soundpub_dash.royalties;
CREATE POLICY "Whitelabels can view royalties for their artists"
  ON soundpub_dash.royalties FOR SELECT TO authenticated
  USING (soundpub_dash.has_role(auth.uid(), 'whitelabel') AND label_user_id = auth.uid());

-- ---- 7) prevent_profile_privilege_escalation: service role bypass (2026-06-19) ----
CREATE OR REPLACE FUNCTION soundpub_dash.prevent_profile_privilege_escalation()
RETURNS trigger
...
BEGIN
  IF auth.uid() IS NULL OR current_setting('role', true) = 'service_role' THEN
    RETURN NEW;
  END IF;
  IF soundpub_dash.is_admin(auth.uid()) THEN
    RETURN NEW;
  END IF;
...
-- =====================================================
-- =====================================================
-- BAGIAN FINAL: API GRANTS UNTUK SUPABASE / POSTGREST
-- =====================================================
-- Wajib untuk schema non-public agar tabel, sequence, dan function dapat diakses melalui Supabase API.
-- RLS tetap menjadi pengaman utama akses data.

GRANT USAGE ON SCHEMA soundpub_dash TO anon, authenticated, service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA soundpub_dash TO anon, authenticated, service_role;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA soundpub_dash TO anon, authenticated, service_role;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA soundpub_dash TO anon, authenticated, service_role;

ALTER DEFAULT PRIVILEGES IN SCHEMA soundpub_dash GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA soundpub_dash GRANT USAGE, SELECT ON SEQUENCES TO anon, authenticated, service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA soundpub_dash GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role;
