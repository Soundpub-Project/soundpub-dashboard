-- =====================================================
-- SoundPub Dashboard - Full Database Schema Export
-- Generated for migration to external Supabase project
-- =====================================================

-- 1. Create Custom Types (ENUM)
-- =====================================================
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.royalties (
  ...
  pendapatan_label_artis NUMERIC NOT NULL DEFAULT 0,
  pendapatan_bersih_soundpub NUMERIC NOT NULL DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE POLICY "Artists can view their royalties" ON public.royalties
  ...
  USING (
    has_role(auth.uid(), 'artist') AND (
      artist_name = get_user_full_name(auth.uid()) OR artist = get_user_full_name(auth.uid())
    )
  );
...
-- =====================================================
INSERT INTO storage.buckets (id, name, public) VALUES ('release-covers', 'release-covers', false) ON CONFLICT DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('track-audio', 'track-audio', false) ON CONFLICT DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('track-video', 'track-video', false) ON CONFLICT DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('audio-clips', 'audio-clips', true) ON CONFLICT DO NOTHING;

-- Storage policies for release-covers
...
CREATE POLICY "Admins can upload release covers" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'release-covers' AND is_admin(auth.uid()));

CREATE POLICY "Labels can upload release covers" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'release-covers' AND has_role(auth.uid(), 'label'));

CREATE POLICY "Admins can update release covers" ON storage.objects
  FOR UPDATE TO authenticated USING (bucket_id = 'release-covers' AND is_admin(auth.uid()));

CREATE POLICY "Labels can update their release covers" ON storage.objects
  FOR UPDATE TO authenticated USING (bucket_id = 'release-covers' AND has_role(auth.uid(), 'label'));

CREATE POLICY "Admins can delete release covers" ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'release-covers' AND is_admin(auth.uid()));
...
CREATE POLICY "Admins can upload track audio" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'track-audio' AND is_admin(auth.uid()));

CREATE POLICY "Labels can upload track audio" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'track-audio' AND has_role(auth.uid(), 'label'));

CREATE POLICY "Admins can delete track audio" ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'track-audio' AND is_admin(auth.uid()));

-- Storage policies for track-video
...
CREATE POLICY "Admins can upload track video" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'track-video' AND is_admin(auth.uid()));

CREATE POLICY "Labels can upload track video" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'track-video' AND has_role(auth.uid(), 'label'));

CREATE POLICY "Admins can delete track video" ON storage.objects
  FOR DELETE TO authenticated USING (bucket_id = 'track-video' AND is_admin(auth.uid()));

-- Storage policies for audio-clips (public)
CREATE POLICY "Anyone can view audio clips" ON storage.objects
  FOR SELECT USING (bucket_id = 'audio-clips');

CREATE POLICY "Admins can upload audio clips" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'audio-clips' AND is_admin(auth.uid()));

CREATE POLICY "Labels can upload audio clips" ON storage.objects
  FOR INSERT TO authenticated WITH CHECK (bucket_id = 'audio-clips' AND has_role(auth.uid(), 'label'));

-- 9. Insert Default App Settings
-- =====================================================
INSERT INTO public.app_settings (key, value) VALUES ('ga4_measurement_id', NULL) ON CONFLICT (key) DO NOTHING;
